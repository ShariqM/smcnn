local PairwiseBatchDistance, parent = torch.class('nn.PairwiseBatchDistance', 'nn.Module')

function PairwiseBatchDistance:__init(p)
   parent.__init(self)
...
function PairwiseBatchDistance:updateOutput(input)
   ...
   elseif input[1]:dim() == 3 then
      ...
      self.output:resize(input[1]:size(1), input[1]:size(2))
      self.output:zero()
      self.output:add(diff:pow(self.norm):sum(3))
      self.output:pow(1./self.norm)
   else
...
function PairwiseBatchDistance:updateGradInput(input, gradOutput)
   if input[1]:dim() > 3 then
      error('input must be vector, matrix or 3d Tensor')
   end
