...
cqt_features = 175
dofile('build_pwdata.lua')

-- Arguments to LSTM (Hacky)
char_vec_size = char_vecs:size()[1]
vec_of_ones = torch.ones(char_vec_size, 1)
vec_inc_u  = torch.zeros(char_vec_size, 1)
for u=1, char_vec_size do
    vec_inc_u[{u,{}}] = u
end
...
if opt.load_net then
    ...
    protos.lstm = LSTM.lstm_window(opt.rnn_size, char_vec_size)
...
function feval(params_)
    ...
    local lstm_w = {[0]=char_vecs[{{},1}]} -- window function
    local lstm_k = {[0]=1} -- Start from beginning
...
function test()
    ...
    for t=1, trainset:size()[1] - 1 do
        x = 1
        lstm_c, lstm_h = unpack(clones.lstm[x]:forward{trainset[{t,{}}], lstm_c, lstm_h})
